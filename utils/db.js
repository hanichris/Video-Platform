...
class DBClient {
  ...
  async getVideoSearch ( q ) {
    try {
      const videos = await dbClient.files.find({}).toArray();
      const filteredVideos = videos.filter( video => {
        const { name, description, tags, title, fileId } = video;
        return name.includes( q ) || description.includes( q ) || tags.includes( q ) || title.includes( q ) || _id.includes( new ObjectId(q) );
      } );
